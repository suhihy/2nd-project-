...
# 연령대별 전체 평균 계산
overall_avg = df[age_columns].mean().reset_index()
overall_avg.columns = ['연령대', '전체평균']
print(overall_avg)
...
# 전체 평균 추가 (검은색 실선)
sns.lineplot(x='연령대', y='전체평균', data=overall_avg, color='black', label='전체 평균', linestyle='--')

# y축 범위 설정
plt.ylim(0, 800000)

# 그래프 레이블 설정
plt.title('채널별 연령대 시청자 수 분포 및 전체 평균')
plt.xlabel('연령대')
plt.ylabel('평균 시청자수')
