...
plt.plot(time_counts['Time_Bin'].astype(str), time_counts['comment_count'], marker='o', linestyle='-', color='b')

# 각 데이터 포인트에 댓글 수 레이블 추가 (댓글 수가 10개 이상인 경우에만)
# for i, row in time_counts.iterrows():
#     if row['comment_count'] >= 70:
#         plt.text(row['Time_Bin'].strftime('%H:%M'), row['comment_count'], 
