...
# '프로그램명' 열에서 '금쪽같은내새끼'가 포함된 행 필터링
filtered_df = df[df['프로그램명'].str.contains('금쪽같은내새끼')]
...
sns.barplot(x='연령대_성별', y='시청률', hue='프로그램명', data=melted_df, palette='viridis')

# 그래프 제목과 레이블 설정
plt.title('Ratings for "금쪽같은내새끼" by Age and Gender')
...
# 그래프를 파일로 저장
plt.tight_layout()
plt.savefig('ratings_plot.png', dpi=300)  # dpi=300은 고해상도 설정
